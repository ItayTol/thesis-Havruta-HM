# -*- coding: utf-8 -*-
"""
Created on Thu Jun 19 19:32:45 2025
...
from havruta_HM_thesis_classification_prep import extract_trait_level_matched_behaviour_and_rules, evaluate_few_shot, evaluate_zero_shot
from collections import Counter

def classify_zero_and_few_shot(personality_trait, posts_df, extra_prompt_few_shot, rep, fold):    
    results = []
    true_labels = []
...
def classifier_using_rules(personality_trait, posts_df, rules, iteration=None):    
    ...
    for _, row in posts_df.iterrows():
        ...
        true_label = row[personality_trait]    
        true_labels.append(true_label)
        result_pred_rules = extract_trait_level_matched_behaviour_and_rules(personality_trait, post1, post2, rules)
        sample_rules_pred = result_pred_rules["classification"]     
        list_rules_pred.append(sample_rules_pred)
        ...
        matched_rules = [entry["rule"] for entry in result_pred_rules["matched_rules_and_behaviours"]]
        matched_behaviours = [entry["behaviour"] for entry in result_pred_rules["matched_rules_and_behaviours"]]
        results_for_rules.append({
            ...
            "matched_behaviours": matched_behaviours,
            "pred_zero_shot": sample_zero_pred,
            "pred_rule_based": sample_rules_pred,
        })    
